...
use gpui::{
    ...
    HighlightStyle, InteractiveElement as _, IntoElement, Length, ListOffset, ListState, ObjectFit,
...
use crate::{
    ...
    text::{
        CodeBlockActionsFn, CodeBlockRendererFn, ImageActivationHandlerFn, ImageInfo,
...
impl From<&ImageNode> for ImageInfo {
    fn from(image: &ImageNode) -> Self {
        Self {
            url: image.url.clone(),
            link_url: image.link.as_ref().map(|link| link.url.clone()),
...
impl Paragraph {
    fn visit_search_text(&self, visit: &mut impl FnMut(&Arc<Mutex<InlineState>>, &str)) {
        let mut text = String::new();
        for child in &self.children {
            text.push_str(&child.text);
            if child.image.is_some() && !text.is_empty() {
                visit(&child.state, &text);
                text.clear();
            }
        }
        if !text.is_empty() {
            visit(&self.state, &text);
        }
    }
...
impl CodeBlock {
    ...
    /// Get the byte range of the block in the original source, including its
    /// fences when the block was parsed from fenced Markdown.
    ...
    fn render(
        ...
        if let Some(renderer) = &node_cx.code_block_renderer {
            if let Some(element) = renderer(self, window, cx) {
                let has_search = self.state.lock().unwrap().search.is_some();
                return div()
                    .v_flex()
                    ...
                    .when(has_search, |this| {
                        this.child(
                            div()
                                .p_3()
                                .border_l_4()
                                .border_color(gpui::rgb(0x3730a3))
...
pub(crate) struct NodeContext {
    ...
    pub(crate) image_activation_handler: Option<Arc<ImageActivationHandlerFn>>,
}
...
impl PartialEq for NodeContext {
    fn eq(&self, other: &Self) -> bool {
        ...
        // Note: render callbacks are intentionally not compared (closures can't be compared)
    }
}
...
impl Node {
    pub(super) fn search_item(&self, query: &str, index: usize) -> Option<usize> {
        let mut remaining = index;
        let nodes = match self {
            Node::Root { children } => children.as_slice(),
            _ => std::slice::from_ref(self),
        };
        for (item, node) in nodes.iter().enumerate() {
            let mut found = false;
            node.visit_search_text(&mut |_, text| {
                if found {
                    return;
                }
                let count = super::search::search_ranges(text, query).len();
                if remaining < count {
                    found = true;
                } else {
                    remaining -= count;
        ...
        match self {
            ...
            | Node::ListItem { children, .. } => {
                for child in children {
                    child.visit_search_text(visit);
                }
            }
            Node::Paragraph(p) | Node::Heading { children: p, .. } => p.visit_search_text(visit),
            Node::Table(table) => {
                for row in &table.children {
                    for cell in &row.children {
                        cell.children.visit_search_text(visit);
                    }
                }
            }
            Node::CodeBlock(code) => visit(&code.state, &code.code()),
            _ => {}
        }
    }

    pub(super) fn prepare_search(&self, request: Option<&super::search::SearchRequest>) {
        let mut index = 0;
        self.visit_search_text(&mut |state, text| {
            let mut state = state.lock().unwrap();
            state.search = None;
            if let Some(request) = request {
                for range in super::search::search_ranges(text, &request.query) {
                    if index == request.index {
                        state.search = Some(super::search::SearchPaint {
                            range,
    ...
    pub(super) fn root_item_for_heading(&self, heading_index: usize) -> Option<usize> {
        let Node::Root { children } = self else {
            return None;
        };
        ...
        children
            .iter()
            .enumerate()
            .filter(|(_, node)| matches!(node, Node::Heading { .. }))
            .nth(heading_index)
...
#[cfg(test)]
mod tests {
    use super::{ImageInfo, ImageNode, LinkMark, Node, Paragraph};
    use gpui::SharedUri;

    #[test]
    fn resolves_heading_index_to_virtual_list_item() {
        let root = Node::Root {
            children: vec![
        ...
        assert_eq!(root.root_item_for_heading(0), Some(1));
        assert_eq!(root.root_item_for_heading(1), Some(3));
        assert_eq!(root.root_item_for_heading(2), None);
        assert_eq!(Node::Divider.root_item_for_heading(0), None);
    }

    #[test]
    fn image_info_preserves_activation_metadata() {
        let image = ImageNode {
            url: SharedUri::from("figure.png"),
            link: Some(LinkMark {
        ...
        let info = ImageInfo::from(&image);

        assert_eq!(info.url.as_ref(), "figure.png");
        assert_eq!(
            info.link_url.as_ref().map(ToString::to_string).as_deref(),
            Some("https://example.com")
        );
        assert_eq!(
            info.title.as_ref().map(ToString::to_string).as_deref(),
            Some("Title")
        );
        assert_eq!(
            info.alt.as_ref().map(ToString::to_string).as_deref(),
            Some("Alt")
        );
    }
}
    ...
    fn render(&self, node_cx: &NodeContext, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        for inline_node in children {
            ...
            if let Some(image) = &inline_node.image {
                ...
                let image_element = img(image.url.clone())
                    .id(ix)
                    .debug_selector(|| "text-view-image".into())
                    .object_fit(ObjectFit::Contain)
                    .max_w(relative(1.))
                    .when_some(image.width, |this, width| this.w(width));
                let handler = node_cx.image_activation_handler.clone();
                let link_url = image.link.as_ref().map(|link| link.url.clone());
                if handler.is_some() || link_url.is_some() {
                    let focus_handle = window
                        .use_keyed_state(("text-view-image-focus", ix), cx, |_, cx| {
                            cx.focus_handle()
                        })
                        .read(cx)
                        .clone()
                    ...
                    let tooltip = match image.title() {
                        ...
                        _ if handler.is_some() => "Click to view image".to_owned(),
                        _ => String::new(),
                    };
                    child_nodes.push(
                        image_element
                            ...
                            .on_click(move |event, window, cx| {
                                let handled = handler
                                    .as_ref()
                                    .is_some_and(|handler| handler(&info, event, window, cx));
                                if handled {
                                    cx.stop_propagation();
                                } else if let Some(link_url) = &link_url {
                                    cx.stop_propagation();
                    ...
                    child_nodes.push(image_element.into_any_element());
                }
