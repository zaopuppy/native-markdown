...
pub(crate) struct InlineState {
    ...
    pub(super) search: Option<super::search::SearchPaint>,
}
...
impl Element for Inline {
    ...
    fn request_layout(
        ...
        if let Some(search) = &self.state.lock().unwrap().search {
            self.highlights = gpui::combine_highlights(
            ...
            .collect();
            // The active match is painted separately below. Prevent a formatted span (notably
            // inline code) from repainting its own background over that marker, and make the
            // foreground deterministic instead of blending it with the span's text color.
            for (range, highlight) in &mut self.highlights {
                if range.start < search.range.end && search.range.start < range.end {
                    highlight.background_color = None;
                    highlight.color = Some(gpui::rgb(0xffffff).into());
                }
            }
            search.request.handle.0.lock().unwrap().background_color =
                Some(gpui::rgb(0x3730a3).into());
        }
    ...
    fn prepaint(
        ...
        if let Some(search) = self.state.lock().unwrap().search.clone() {
            let layout = self.styled_text.layout();
            if let (Some(start), Some(end)) = (
                layout.position_for_index(search.range.start),
                layout.position_for_index(search.range.end),
            ) {
                let rect = Bounds::from_corners(
                    start,
                    point(end.x.max(start.x + px(2.)), end.y + layout.line_height()),
                );
                let mut geometry = search.request.handle.0.lock().unwrap();
                geometry.text = self.text[search.range.clone()].to_owned().into();
                geometry.bounds = Some(rect);
                geometry.painted = false;
            }
        }
    ...
    fn paint(
        ...
        for (range, highlight) in &self.highlights {
            ...
            while cursor < range.end {
                let next = cursor + self.text[cursor..].chars().next().unwrap().len_utf8();
                if let (Some(start), Some(end)) = (
                    text_layout.position_for_index(cursor),
                    text_layout.position_for_index(next),
                ) {
                    ...
                    let rect = Bounds::from_corners(
                        start,
                        point(
                            right.max(start.x + px(2.)),
                            start.y + text_layout.line_height(),
                        ),
                    );
                    window.paint_quad(quad(
                        rect,
        ...
        if let Some(search) = &state.search {
            // Paint search above formatted-span backgrounds and below its white glyphs.
            let mut cursor = search.range.start;
            let mut painted = false;
            while cursor < search.range.end {
                let next = cursor + self.text[cursor..].chars().next().unwrap().len_utf8();
                if let (Some(start), Some(end)) = (
                    text_layout.position_for_index(cursor),
                    text_layout.position_for_index(next),
                    ...
                    if search
                        ...
                        .geometry()
                        .viewport
                        .is_none_or(|viewport| viewport.intersects(&rect))
                    {
                        painted = true;
                    }
                }
            ...
            search.request.handle.0.lock().unwrap().painted = painted;
        }
        self.styled_text
