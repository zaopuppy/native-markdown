...
impl TextElement {
    ...
    fn match_bounds(
        ...
        let mut offset = layout.visible_range_offset.start;
        let mut y = layout.visible_top;
        for line in layout.lines.iter() {
            if range.start >= offset && range.start <= offset + line.len() {
                let start = line.position_for_index(range.start - offset, layout.line_height)?;
                let end = line
                    .position_for_index((range.end - offset).min(line.len()), layout.line_height)?;
                let origin = bounds.origin + point(layout.line_number_width, y);
                return Some(Bounds::from_corners(
                    origin + start,
                    origin + point(end.x.max(start.x + px(2.)), end.y + layout.line_height),
                ));
            }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        if let Some((range, _, _)) = &state.external_search {
            document_colors.push((range.clone(), gpui::rgb(0x3730a3).into()));
        }
        ...
        let external_search = self.state.read(cx).external_search.clone();
        if let Some((range, request, handle)) = external_search {
            let mut geometry = handle.0.lock().unwrap();
            geometry.painted = false;
            let viewport = self.state.read(cx).input_bounds;
            geometry.viewport = Some(viewport);
            if let Some(rect) = Self::match_bounds(&range, &last_layout, &bounds) {
                ...
                geometry.text = self
                    ...
                    .to_string()
                    .into();
                geometry.painted = viewport.intersects(&rect);
                if geometry.centered_request != Some(request) {
                    geometry.centered_request = Some(request);
                    let delta = rect.center().y - viewport.center().y;
                    let input = self.state.clone();
                    window.defer(cx, move |_, cx| {
                        input.update(cx, |input, cx| {
                            let mut offset = input.scroll_handle.offset();
                            offset.y -= delta;
                            input.update_scroll_offset(Some(offset), cx);
                        });
                    });
                }
            } else if geometry.reveal_request != Some(request) {
                geometry.reveal_request = Some(request);
                let input = self.state.clone();
                window.defer(cx, move |_, cx| {
                    input.update(cx, |input, cx| input.scroll_to(range.start, None, cx))
                });
            }
        }
