...
pub struct InputState {
    ...
    pub(super) external_search: Option<(Range<usize>, u64, crate::text::SearchHandle)>,
...
impl InputState {
    ...
    /// Highlight and reveal an external search occurrence without stealing keyboard focus.
    pub fn set_search_match(
        &mut self,
        range: Option<Range<usize>>,
        request: u64,
        handle: crate::text::SearchHandle,
        cx: &mut Context<Self>,
    ) {
        self.external_search = range
            .filter(|r| r.start < r.end && r.end <= self.text.len())
            .map(|r| (r, request, handle));
        if let Some((range, _, _)) = &self.external_search {
